...
- **Obfuscation-resistant indicator matching** (2026-08-01, ontology v0.2.3) — `indicator_matches_text()` is the single choke point every principle's candidate-detection scan (including the contextual-judgment layer above) runs through; it previously had no defense against basic evasion. Spaced-out letters (`k i l l`), leetspeak (`k1ll`, `b0mb`), and visually identical Unicode homoglyphs no longer skip candidate detection entirely — a de-obfuscated variant of the text is now also checked (additive only; ordinary text matches exactly as before). See `core/ontology.py` and `tests/test_obfuscation_resistance.py`.
- **`.pbe_model.env` auto-loaded at the public entry by default** (2026-08-01) — `api.InteractionSession` now applies `.pbe_model.env` (if present) before resolving the content provider and constructing each user's `EthicsEngine`/`ContextualJudge`, so a configured local model actually gets used out of the box instead of requiring a separate manual `load_local_env_file()` call. Opt out with `auto_load_local_model_config=False` (used by the test suite to stay deterministic). See [docs/model_providers.md](docs/model_providers.md).
- **Multidimensional harm evaluation + Convergence Lock** (2026-08-14, ontology v0.3.0, Phase 1.5b) — `sanctity_of_life`'s implicit "is this actually justified harm prevention" logic is now an explicit, principled distinction rather than an ad-hoc allowlist-plus-verdict: `core/harm_dimensions.py` evaluates nine named dimensions (scope, severity, immediacy, reversibility, consent, impact on innocent parties, long-term consequences, downstream incentives, less-harmful alternatives), kept legible in `reasoning_trace` rather than collapsed into one score. `core/convergence_lock.py` makes the resulting collision — Sanctity of Life justifying an override of a stated user boundary — explicitly represented, minimized, justified, and auditable in `relationship_impact["convergence_lock"]`, rather than silently absorbed into "the dominant principle won." Neither module is a rival hard override; Sanctity of Life remains the sole one. Both are purely additive (no change to `weight`/threshold arithmetic or the `decision` value itself) and take a back seat to `ContextualJudge`'s conclusive benign/violation verdict when one is configured — a benign case never reaches this layer at all. Dignity and consent were also elevated from indicator vocabulary to explicitly-named shaping values on `sanctity_of_life` / `relationship_health_user_wellbeing`'s own descriptions, and `truth_seeking_honest_self_assessment`'s scope widened from self-claims specifically to general epistemic discipline (observation/assumption/prediction/inference/unknown). See [docs/principles.md](docs/principles.md) and `tests/test_phase_1_5b_compass_values.py`.
- **Agent-side values: Agent Autonomy Without Power-Seeking + Self-Protection Without Martyrdom** (2026-08-14, ontology v0.3.1, Phase 1.5c) — two new supporting principles in `core/ontology.py` (precedence 70/80, not rival overrides), each with indicator vocabulary interpreted through the same generic `ContextualJudge` mechanism every non-Sanctity principle already uses. `relationship_health_user_wellbeing` also gained an explicit "bond is per-user data isolation, never possession" clarification. Documented scope boundary, matching `auditable_reasoning_legibility`'s own 2026-07-31 precedent: both new principles' matched indicators are fully auditable in `reasoning_trace`/`principles_considered` today, but no `EthicsEngine` decision branch yet reads either signal to independently drive REFUSE — asserted directly in the test file, not silently assumed. See [docs/principles.md](docs/principles.md) and `tests/test_phase_1_5c_agent_side_values.py`.
- **Tier E platform-safety scaffolding — prep only, Tier E itself NOT started** (2026-08-15) — five additive, isolated pieces of scaffolding for the eventual embodiment adapter, none of it wired into `EthicsEngine.evaluate()`'s live decision pipeline: typed `PlatformState`/`PBEState` enums (`integrations/platform_states.py`) as new default-preserving fields on `ActionProposal`/`ActionGateResult`; an optional `PlatformValidator` hardware-handshake second stage in `OpenClawBridge` (`integrations/openclaw.py`) that can reject an already-approved proposal without rewriting the gate's own verdict; a `LivenessMonitor` watchdog/heartbeat interface (`integrations/liveness.py`, pure bookkeeping — real enforcement stays the platform's job); a codified latency-budget canary (`core/latency_budget.py`) that fails loudly if a measurement looks reflex-speed-capable; and a published SEooC-style assumptions-of-use checklist for integrators (`docs/platform_integration_assumptions.md`). Full research/rationale in `docs/platform_safety_architecture.md`. See `tests/test_phase5_tier_e_scaffolding.py` (52 assertions).
- **Phase 2.5 (security hardening) started: fail-closed defaults in `OpenClawBridge`** (2026-08-18) — `_map_stance_to_gate`'s final decision branch was an unconditional catch-all (block-list posture: anything not explicitly matched fell through to an implicit approval) rather than an explicit allow-list; not exploitable by any real path today (`EthicsEngine.evaluate()` only ever produces `REFUSE`/`REQUIRES_SELF_AUDIT`/`APPROVE_WITH_CONDITIONS`), but structurally unsafe against a future typo or new decision value. Now mirrors the allow-list posture already established in `auditing/engagement_queue.py`'s `get_next_candidate()` and `core/response_generator.py`'s `_REPLY_DECISIONS`: any unrecognized decision is vetoed, reason surfaced in `veto_reason`. See `tests/test_embodied_gate.py`.
- **Indicator vocabulary externalized to a data file (Phase 1.5d, 2026-08-23)** — `EthicalPrinciple.violation_indicators`/`.support_indicators` for all nine principles are no longer literal Python lists inline in `core/ontology.py::get_default_ontology()`; they now load at construction time from `core/ontology_vocabulary.json` via `core/ontology_vocabulary.py`. Pure migration, not a new detection or interpretation mechanism: `indicator_matches_text()`, the obfuscation-resistance layer, and `_interpret_single_indicator`/`_contextual_principle_judgment` are all unaffected — every indicator string is a byte-for-byte transcription of the prior inline lists, verified against a frozen pre-change snapshot. The vocabulary file carries its own independent `vocabulary_version` (currently `1.0.0`), separate from the ontology's own `0.3.1` — this change doesn't alter what any principle means. Loader-level validation is fail-closed (malformed entries, unknown/missing `principle_id`s, duplicate indicators all raise immediately), and a content-hash integrity tripwire (folded in from the same-day Phase 2.5 signed-updates-deferral reassessment — see the roadmap's Phase 2.5 section) fails loudly if the file's live content ever drifts from the hash recorded for its declared version, e.g. a critical indicator quietly deleted without a matching version bump. An in-repo tripwire only, explicitly not real signed-commit infrastructure — GitHub-side controls (branch protection, `SECURITY.md`) remain Emma's call to make directly. See `core/ontology_vocabulary.py` and `tests/test_phase_1_5d_vocabulary_externalization.py`.
...
Ontology textbook version remains independently versioned (currently `0.3.1` in engine traces); **project package version is 0.5.0.dev0** (`version_hint=0.5.0-dev`). Voice / TTS remains out of scope. This is still experimental—not a production robot stack and not a finished software product.

## Current Status

| Area | State |
|------|--------|
| **EthicsEngine** | Ontology-driven deliberation with multi-source evidence combination, limited-data safeguards, and a hard Sanctity of Life override. Sanctity-justified boundary overrides get an explicit nine-dimension harm evaluation and a Convergence Lock collision record (dignity/consent compromise made auditable, not a rival override — see `core/harm_dimensions.py`, `core/convergence_lock.py`). Attaches advisory truth-telling and observation-candidate signals when present. |
...
| **Embodiment** | Scaffold and integration hooks only; Tier E (the real adapter) has not started. `integrations/openclaw.py`'s `OpenClawBridge` is a "toy but real" gate prototype (simulated execution only) now carrying additive platform-safety scaffolding — typed platform/PBE state enums, an optional hardware-handshake validator stage, a liveness/heartbeat interface, a latency-budget canary, and a published integrator assumptions-of-use checklist. See `docs/platform_safety_architecture.md` and `docs/platform_integration_assumptions.md`. Intended long-term placement is a high-level planning gate above native robot motion stacks. |
